<template>
  <v-container fluid="fluid" style="padding: 0px">
    <v-card shaped elevation="0" color="black" dark>
      <v-layout style="margin-bottom: 1px">
        <v-flex md1="md1" style="display: inline-block">
          <h3 style="margin: 0px; text-align: center; padding-top: 7px">
            <strong>정영철 교수</strong>님 <v-spacer></v-spacer>환영합니다.
          </h3>
        </v-flex>
        ...
        <v-flex md1="md1">
          <div style="text-align: center; padding-top: 7px"></div>
        </v-flex>
        <v-flex md6="md6" style="text-align: center; padding-top: 10px">
          <h1>
            <a v-on:click="onClickPrev(currentYear, currentMonth)">◀</a>
            {{ currentYear }}년 {{ currentMonth }}월
            <a v-on:click="onClickNext(currentYear, currentMonth)">▶</a>
          </h1>
        </v-flex>
        <v-flex md1.5="md1.5" style="text-align: right; padding-top: 6px">
          <template>
            <div class="text-center">
              ...
              >
                <template v-slot:activator="{ on }">
                  <v-btn x-large="" color="red" text v-on="on">
                    <strong>공휴일/시간 설정</strong>
                  </v-btn>
                </template>
                <v-card>
                  <v-btn @click="holidayAdd()" style="margin: 15px"
                    >공휴일 설정
                  </v-btn>
                  <v-btn @click="holidayDelete()">공휴일 해제</v-btn>
                  <v-btn @click="TimeSet()" style="margin: 15px"
                    >시간 설정</v-btn
                  >
                  <v-menu
                    ref="menu3"
                  ...
                  <v-card-actions>
                    ...
                    <v-btn
                      color="primary"
                      @click="$refs.menu3.save(dates), update(dates)"
                      >OK</v-btn
                    >
                  </v-card-actions>
                </v-card>
...
export default {
  ...
  methods: {
    update(dates) {
      ...
      axios
        .post(
          "http://ec2-13-209-70-126.ap-northeast-2.compute.amazonaws.com/web/professor/CRUD/HolidayCRUD.php",
          [
            this.dates,
            this.set,
            this.rel,
    ...
    forr() {
      ...
      for (
        var k = 0;
        k < new Date(this.currentYear, this.currentMonth, 0).getDate();
        k++
      ) {
    ...
    change2(namelist) {
      ...
      axios
        .post(
          "http://ec2-13-209-70-126.ap-northeast-2.compute.amazonaws.com/web/professor/CRUD/StudentCRUD.php",
          namelist
        )
        .then((res) => {
          this.datalist = res.data;
